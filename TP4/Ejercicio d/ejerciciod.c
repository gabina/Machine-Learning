/*
knn.c: algoritmo vecinos a distancia d.
...
float D;			/* distancia maxima a considerar*/
...
arquitec (char *filename)
{
  ...
  printf ("\nDistancia máxima considerada: %f", D); 
...
void distancia (double *input)
{
  ...
  for(patron = 0; patron < PR; patron ++){
	...
	//printf("distancia: %f ",(dist[patron]).distancia);
	}
...
output (double *input, int d)
{
  ...
  for(i = 0; i < N_Class; i++)
   ...
   /*Cuento cuántos vecinos a distancia menor que dist hay de cada clase*/
   for(k = 0; k < PR; k++){
      if((dist[k]).distancia < d)
	     clases[(dist[k]).clase] ++;
	  else
	     break;
	}
	...
	/* Si la cantidad de puntos a distancia menor a d es mayor a 0, actualizo la nueva distancia. Si es igual a 0, retorno la clase del mas cercano*/
  	if (k != 0)
		nueva_dist = (dist[k-1]).distancia;
	else
		return (dist[0].clase); 
  ...
  if(cant_ganadoras == 1)
   ...
   return output(input, nueva_dist);
}
...
/* ------------------------------------------------------------------------------ */
double
propagar (double **S, int pat_ini, int pat_fin, int usar_seq)
{
