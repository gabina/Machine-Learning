...
int CCLAS;			/* CCLAS: Cantidad de clases existentes. Se sobreentiende que son naturales consecutivos crecientes y comienzan en 0.*/ 
  ...
  for (patron=pat_ini; patron < pat_fin; patron ++) {
   ...
   /*nu tiene el numero del patron que se va a presentar*/
    ...
    for(i = 1; i <= N3; i++) {
      ...
      /*calcula error discreto para una sola salida - problema binario
      if(N3==1) 
...
/* ------------------------------------------------------------------------------ */
/*propagar2: es igual a propagar, exceptuando que genera un archivo "predicciones_discretas"
 * en el cual aparecen la clase a la cual pertenece cada predicción.
 * Esta función es llamada para calcular las predicciones discretas para el conjunto de test.
 *                 */
/* ------------------------------------------------------------------------------ */
float propagar2(float **S,int pat_ini,int pat_fin,int usar_seq){
  ...
  fprueba = fopen("predicciones_discretas","w");
...
int train(char *filename){
  ...
  sinapsis_read(WTS+1);
  mse_test=propagar2(test,0,PTEST,0);
  disc_test=discrete_error;
  for(k=0; k < PTEST ; k++){
