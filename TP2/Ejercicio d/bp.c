...
int train(char *filename){
  ...
  float penalty; // En esta variable se calcula el término de penalización
  /* Inicializar archivos de control */
  sprintf(filepat,"%s.predic",filename);
  fpredic=fopen(filepat,"a");
  error=(fpredic==NULL);
  if(error){
  ...
  for (iter=1; iter<=ITER; iter++) {
	...
	/* Inicializo el término de penalización en 0*/
    ...
    for(nu1=0;nu1<PR;nu1++) {
	  ...
	  for( i = 1 ;i<= N3;i++) 
		for( j = 0 ;j<= N2;j++){
			...
			w2[i][j] = w2[i][j] * (1 - 2 * GAM * eta) + dw2[i][j]; // Modificación a la actualización para penalizar pesos grandes
			penalty = penalty + pow(w2[i][j],2.0); // Acumulo los pesos al cuadrado
			//w2[i][j] += dw2[i][j];
		}
	  ...
	  for( j = 1 ;j<= N2;j++) 
		for( k = 0 ;k<= N1;k++){
			...
			w1[j][k] = w1[j][k] * (1 - 2 * GAM * eta) + dw1[j][k]; // Modificación a la actualización para penalizar pesos grandes
			//w1[j][k] += dw1[j][k];
			penalty = penalty + pow(w1[j][k],2.0); // Acumulo los pesos al cuadrado 
		}
	...
	/* Multiplico por gamma la suma de los cuadrados de los pesos para obtener el término de penalización */
	penalty = penalty * GAM;
    ...
    if ((iter/NERROR)*NERROR == iter) {
      ...
      fprintf(ferror,"%f,%f,%f,%f,%f,",mse,mse_train,mse_valid,mse_test,penalty);
