...
{
	p<-read.csv("/home/gabina/Facultad/Machine Learning/TP FINAL/heladas/heladasEscaladas.data")
	...
	for(i in 1:x){
		#Escribo clase 0
		name <- paste('/home/gabina/Facultad/Machine Learning/TP FINAL/svm_light/heladasSVM/heladasEscaladas',i,'.data',sep="")
		init <- c0 * (i - 1) + 1
		fin <- c0 * i
		for( j in init:fin){	
	...
	if (c0 * x != prop0 * cant){
		fin <- prop0 * cant - c0 * x
		for(i in 1:fin){
			name <- paste('/home/gabina/Facultad/Machine Learning/TP FINAL/svm_light/heladasSVM/heladasEscaladas',x - i + 1,'.data',sep="")
	...
	for(i in 1:x){
		...
		name <- paste('/home/gabina/Facultad/Machine Learning/TP FINAL/heladas/heladasEscaladas',i,'.data',sep="")
		init <- c0 * (i - 1) + 1
		fin <- c0 * i
		for( j in init:fin){	
	...
	if (c0 * x != prop0 * cant){
		fin <- prop0 * cant - c0 * x
		for(i in 1:fin){
			name <- paste('/home/gabina/Facultad/Machine Learning/TP FINAL/heladas/heladasEscaladas',x - i + 1,'.data',sep="")
