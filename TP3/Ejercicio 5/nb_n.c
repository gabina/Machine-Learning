/*
nb_n.c : Clasificador Naive Bayes usando histogramas
...
double **histograma; /* histograma[b][c] contiene cuántos puntos pertenecientes al cuadrante b son de clase c*/
...
/*ALLOCAR ESPACIO PARA LAS MATRICES DEL ALGORITMO*/
	...
	histograma = (double**)calloc(pow(N_BINS,N_IN),sizeof(double*));	

	for(i = 0; i < pow(N_BINS,N_IN); i++){
		histograma[i] = (double *)calloc(N_Class,sizeof(double*));		
...
/*------------------------------------------------------------------------------ */
/* convertir: traduce un entero que puede tomar valores desde 0 hasta pow(N_BINS,N_IN)
 * en el cuadrante correspondiente.                                               */
/* ------------------------------------------------------------------------------ */

int* convertir(int n){
	int *conversion = malloc(sizeof(int)*N_IN);
	
	conversion[0] = n % N_BINS;
	conversion[1] = N_BINS - (n / N_BINS)-1;
...
int *conv;
	...
	for(j=0;j<pow(N_BINS,N_IN);j++){
		conv = convertir(j);
		pertenece = 1;	
		/* Itero en los atributos */
		for(i = 0; i < N_IN; i++){
			extr_izq = minimo[i] + longitud[i] * conv[i];
			extr_der = extr_izq + longitud[i];
			/* Si el punto se encuentra entre los dos extremos*/	
			if (!(x[i]>= extr_izq && x[i] <= extr_der)){
				/* Desctivo la bandera pertenece */
				pertenece = 0;
				break;
			}
		}
		if(pertenece)
			return histograma[j][clase];
	}
}
...
int train(char *filename){
	...
	*/
	...
	for (patron=0; patron < PR; patron ++) {
		...
		/* Itero en los cuadrantes */
		for (j = 0; j < pow(N_BINS,N_IN); j++){
			conv = convertir(j);
			for(m = 0;m<N_IN;m++)
				//printf("\nBin: %d Cuadrante %d ",j,conv[m]);	
			pertenece = 1;
			...
			for(i = 0; i < N_IN; i++){
				...
				//printf("\n %f %f",extr_izq,extr_der);
				/* Si el punto se encuentra entre los dos extremos */	
				if (!(data[nu][i]	>= extr_izq && data[nu][i] <= extr_der)){
						/* Desactivo la bandera pertenece */
						pertenece = 0;
						break;
				}
			}
			if(pertenece){
				clase = (int) data[nu][N_IN];
				histograma[j][clase] += 1.0;
				//printf("\nPertenece a cuadrante %d",j);
				break;
			}
		}
	}
	...
	for(j =0; j< pow(N_BINS,N_IN);j++){
		...
		/* Sumo +1 correspondiente a mp */
				//printf("\n %d %d %f",j,k,histograma[j][k]);
				histograma[j][k] += 1.0;
				/* Divido por n + m*/
				histograma[j][k] /= ((double) PR * probTrain[k] + M);	
				//printf("\n Bin %d Clase %d: %f",j,k,histograma[j][k]);
